...
/**
 * Class for representing colors in RGB and HSV color modes. Allows for quick conversion between the
 * two modes. All values are in ranges supported by the WPILib {@code AddressableLED} class. RGB
 * values are in the range [0-255]. HSV values are in the range [0-180), [0-255], [0-255]. Intended
 * to be used with {@code PWMLEDController} and {@code LEDStrip} classes.
 *
 * @see <a href="https://en.wikipedia.org/wiki/HSL_and_HSV">HSV on Wikipedia</a>
 * @see edu.wpi.first.wpilibj.AddressableLED
 * @see com.argsrobotics.lib.led.PWMLEDController
...
public class Color {
  ...
  private static HSV RGBtoHSV(int r, int g, int b) {
    ...
    return new HSV(hue / 2, (int) Math.round(saturation * 255), (int) Math.round(cMax * 255));
  }
  ...
  /**
   * Represents a color in HSV. With hue in the range [0-180), saturation in the range [0-255], and
