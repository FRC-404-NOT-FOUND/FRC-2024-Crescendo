...
public class PWMLEDController {
  ...
  /**
   * Sets the given LED pattern to all LED strips.
   *
   * @param pattern the LED pattern to set
   */
  public void setAllStripPatterns(LEDPattern pattern) {
  ...
  /**
   * Sets the primary color for all LED strips.
   *
   * @param color the color to set as the primary color for all LED strips
   */
  public void setAllStripPrimaryColors(Color color) {
    for (LEDStrip strip : strips) {
      strip.setPrimaryColor(color);
    }
  }

  /**
   * Sets the secondary color for all LED strips.
   *
   * @param color the color to set as the secondary color
   */
  public void setAllStripSecondaryColors(Color color) {
    for (LEDStrip strip : strips) {
      strip.setSecondaryColor(color);
    }
  }

  /**
   * Adds an LED strip to the controller. This is a relatively expensive call, so it should be
   * avoided in the loop. If a new strip is added between two existing strips, all strips after the
   * new strip will have their index shifted. But the lengths and offsets will be unaffected.
   *
  ...
  /**
   * Adds a new LED strip to the controller with a certain length and offset from the start. This is
   * a relatively expensive call, so it should be avoided in the loop. If a new strip is added
   * between two existing strips, all strips after the new strip will have their index shifted. But
   * the lengths and offsets will be unaffected.
   *
  ...
  /**
   * Adds a new LED strip to the controller with a certain length right after the last strip. This
   * is a relatively expensive call, so it should be avoided in the loop.
   *
   * @param length The length of the strip in LEDs.
   * @throws DuplicateLEDAssignmentException If the new strip overlaps with an existing strip.
   */
  public void addStrip(int length) throws DuplicateLEDAssignmentException {
    ...
    if (!strips.isEmpty()) {
      offset =
          strips.get(strips.size() - 1).getOffset() + strips.get(strips.size() - 1).getLength();
    }
  ...
  /**
   * Removes the LED strip at a certain index. This is a relatively expensive call, so it should be
   * avoided in the loop. If the strip that gets removed is not the last strip, all strips after the
   * removed strip will have their index shifted.
   *
  ...
  /** Starts the output of the LED controller. The output writes continuously. */
  public void start() {
  ...
  /** Updates the LED strips. This should be called in the loop. */
