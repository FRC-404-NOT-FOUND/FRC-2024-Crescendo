...
package com.argsrobotics.crescendo2024.subsystems.swerve;
...
public class Swerve extends SubsystemBase implements GenericSwerveDrive {
   ...
   */
  public Command followAuto(String pathName) {
  ...
  public Command followPath(String path) {
    ...
    traj = PathPlannerPath.fromChoreoTrajectory(path);
    if (traj == null) {
      traj = PathPlannerPath.fromPathFile(path);
    }
  ...
  public ChassisSpeeds calculateSlewRate(
    ...
    speeds.vxMetersPerSecond =
        translationMagnitude * Math.cos(translationDirection) * getMaxLinearSpeedMetersPerSec();
    speeds.vyMetersPerSecond =
        translationMagnitude * Math.sin(translationDirection) * getMaxLinearSpeedMetersPerSec();
    speeds.omegaRadiansPerSecond = omega * getMaxAngularSpeedRadPerSec();
   ...
   */
  public void runVelocity(ChassisSpeeds velocity, Translation2d centerOfRotation) {
