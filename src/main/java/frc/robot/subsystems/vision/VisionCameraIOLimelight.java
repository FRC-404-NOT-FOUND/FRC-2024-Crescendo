...
public class VisionCameraIOLimelight implements VisionCameraIO {
    ...
    public void updateInputs(VisionCameraIOInputs inputs) {
        ...
        inputs.timestamp = Timer.getFPGATimestamp() - ((get("cl").getDouble(0) + get("tl").getDouble(0)) / 1000.0);
        ...
        if (inputs.mode == Mode.AprilTag) {
            ...
            if (inputs.hasTarget) {
                double[] poseData = get("targetpose_robotspace").getDoubleArray(new double[6]); // I sure hope this works for target tracking not just apriltags
                Transform3d transform = new Transform3d(
                    new Translation3d(poseData[0], poseData[1], poseData[2]),
                    new Rotation3d(poseData[3], poseData[4], poseData[5]));

                Pose3d pose = new Pose3d(RobotState.getCurrentRobotState().currentPose).transformBy(transform);
